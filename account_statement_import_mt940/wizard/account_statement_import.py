...
class AccountStatementImport(models.TransientModel):
        ...
        for detail in transaction_details.split("+"):
            if detail.startswith('21'):
                ...
                if len(detail_row_22.split(":")) > 1:
                    ...
                    detail_row_22_customer_data = dict(zip(detail_row_22_customer[::2], detail_row_22_customer[1::2]))
    ...
    def _prepare_mt940_transaction_line(self, transaction):
        ...
        vals = {
            ...
            "payment_ref": detail_data.get('21', f"{transaction['customer_reference']}"),
            "amount": float(transaction['amount'].amount),
            "unique_import_id": f"{transaction['id']}-{transaction['customer_reference']}",
    ...
    def _parse_file(self, data_file):
        ...
        try:
            ...
            balance = float(mt940_transactions_data['final_closing_balance'].amount.amount)
            vals_bank_statement = {
            ...
            result.append(
                (mt940_transactions_data['final_opening_balance'].amount.currency, mt940_transactions_data['sequence_number'], [vals_bank_statement])
            )
        except Exception as e:
            raise UserError(
                _(
